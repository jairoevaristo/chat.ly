  ...
  return (
    <div className="w-96 cursor-pointer relative">
      <Menu as="div" className="relative inline-block text-left">
        ...
        >
          <Menu.Items className="absolute top-6 left-1 mt-2 w-48 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="px-1 py-1 ">
              <Menu.Item>
                {({ active }) => (
                  <button
                    className={`${
                      active ? 'text-white bg-gray-500' : 'text-gray-800'
                    } group flex w-full items-center rounded-md px-2 py-2 text-sm`}
                  >
                   
                    <LogoutIcon className={`h-5 w-5 mr-1 ${active ? 'text-white' : 'text-gray-800'}`} />
                    Sair
                  </button>
                )}
              </Menu.Item>
